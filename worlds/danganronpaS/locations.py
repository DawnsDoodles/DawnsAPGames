...
from ..sa2b.Names.LocationName import sand_ocean_region
...
location_dictionary: dict[str, int] = {
    ...
    **{f"Craft {crafted_item.item_name}": i+2501 for i, crafted_item in enumerate(data.CraftedItem)},
    **{f"Clear a Development Plan with {character.char_name}": i+3001 for i, character in enumerate(data.Character)},
    **{f"Defeat {enemy.enemy_name}": i+3501 for i, enemy in enumerate(data.EnemyInformation)},
    **{f"[Eventsanity] - {event_sanity.event_name}": i+16501 for i, event_sanity in enumerate(data.CharacterEvent)},
    ...
    **{f"Collect {x} grimoire in 1 Dev Mode game": i+7001 for i, x in enumerate([*range(1, 6, 1)])},
    ...
    **{f"View {character.char_name}'s Events 10 Times": i+15500 for i, character in enumerate(data.Character)},
    ...
    **{f"Clear a dev plan without resting": 16006},
    ...
    # Cerebral Character (N)
    # 1 prog scroll
    # ~15 Growth +1
    # 2 beneficial events
    # level 25 () int
    # craft WORN_HACKING_GUN (25 int)
    # craft HIGH_SCHOOLERS_UNIFORM (70 influence)

    # influence=70, focus=0, strength=0, stamina=0, intellect=0, endurance=0, agility=0, luck=0)

    # main damage stat 10x
    # stamina 3x
    # endurance
    # agility
    # luck 1x

    # have some items (CollectionState)
    # turns into
    # having some characters
    # having some island access
    # having some equipment
    # having some hype cards

    # gives a number (combat score)
    # each floor requires a number
    # that is combat logic

    # having a character and break damage limit is only relevant past lvl 50 for example



## Actually creating the locations
def create_locations(world):
    for name, id in location_dictionary.items():
        if "gachapull" in name:
            pullnum = name.split("_")
            pullnum = int(pullnum[1])
            if pullnum < 53:
                region = world.get_region("Gacha1")
                location = DRSLocation(world.player, name, id, region)
                region.locations.append(location)
            elif pullnum < 106:
                region = world.get_region("Gacha2")
                location = DRSLocation(world.player, name, id, region)
                region.locations.append(location)
            elif pullnum < 159:
                region = world.get_region("Gacha3")
                location = DRSLocation(world.player, name, id, region)
                region.locations.append(location)
            elif pullnum < 212:
                region = world.get_region("Gacha4")
                location = DRSLocation(world.player, name, id, region)
                region.locations.append(location)
            elif pullnum < 265:
                region = world.get_region("Gacha5")
                location = DRSLocation(world.player, name, id, region)
                region.locations.append(location)
            elif pullnum < 318:
                region = world.get_region("Gacha6")
                location = DRSLocation(world.player, name, id, region)
                region.locations.append(location)
            elif pullnum < 371:
                region = world.get_region("Gacha7")
                location = DRSLocation(world.player, name, id, region)
                region.locations.append(location)
            elif pullnum < 424:
                region = world.get_region("Gacha8")
                location = DRSLocation(world.player, name, id, region)
                region.locations.append(location)
            elif pullnum < 477:
                region = world.get_region("Gacha9")
                location = DRSLocation(world.player, name, id, region)
                region.locations.append(location)
            elif pullnum < 530:
                region = world.get_region("Gacha10")
                location = DRSLocation(world.player, name, id, region)
                region.locations.append(location)
            elif pullnum < 583:
                region = world.get_region("Gacha11")
                location = DRSLocation(world.player, name, id, region)
                region.locations.append(location)
            elif pullnum < 636:
                region = world.get_region("Gacha12")
                location = DRSLocation(world.player, name, id, region)
                region.locations.append(location)
            elif pullnum < 689:
                region = world.get_region("Gacha13")
                location = DRSLocation(world.player, name, id, region)
                region.locations.append(location)
            elif pullnum < 742:
                region = world.get_region("Gacha14")
                location = DRSLocation(world.player, name, id, region)
                region.locations.append(location)
            elif pullnum < 806:
                region = world.get_region("Gacha15")
                location = DRSLocation(world.player, name, id, region)
                region.locations.append(location)
            ...
            num = parts[3]
            if num < 51:
                region = world.get_region("Battle Tower Lower Floors")
                location = DRSLocation(world.player, name, id, region)
                region.locations.append(location)
            elif num < 101:
                region = world.get_region("Battle Tower Upper Floors")
                location = DRSLocation(world.player, name, id, region)
                region.locations.append(location)
            elif num < 151:
                region = world.get_region("Battle Tower Ultra Lower Floors")
                location = DRSLocation(world.player, name, id, region)
                region.locations.append(location)
            elif num < 201:
                region = world.get_region("Battle Tower Ultra Upper Floors")
                location = DRSLocation(world.player, name, id, region)
                region.locations.append(location)
        else:
